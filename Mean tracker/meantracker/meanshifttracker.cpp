...
bool MeanShiftTracker::findTarget(const Mat& candidateImg, Rect& candidateRect)
{
    ...
    while ( (diff != prevDiff || diff > epsilon) && iter < mMaxIteration)
	{
        ...
        if( x >= candidateImg.size().width - width )
        {
            x = (candidateImg.size().width-1) - width;
        }
        if( y >= candidateImg.size().height - height )
        {
            y = (candidateImg.size().height-1) - height;
        }
 ...
 *
 *****************************************************************************/
void MeanShiftTracker::setTargetModel(Mat model)
{
    model.copyTo(mTargetModel);
}
